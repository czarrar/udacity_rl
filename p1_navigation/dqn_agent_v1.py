...
class Agent():
    ...
    def learn(self, experiences):
        ...
        #    next_states is 64x8 where 64 is the batch size
        #    self.qnetwork_target(next_states) is 64x4
        #    detach() returns a tensor copy detached from the graph (no gradient)
        #    max(1)[0] returns the the max value for each row, which is 64 values
        #    usqueeze(1) returns a tensor with an added dimension of 1
        #    Final result is a 64X1 tensor
